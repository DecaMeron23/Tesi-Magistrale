...
typedef struct
{
	uint16_t CustomGeneralHdle; /**< GENERAL handle */
	uint16_t CustomTimeHdle; /**< TIME handle */
	uint16_t CustomPwmHdle; /**< PWM handle */
	uint16_t CustomFrequencyHdle; /**< FREQUENCY handle */
	uint16_t CustomDuty_CycleHdle; /**< DUTY_CYCLE handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeTime = 7;
uint16_t SizeFrequency = 4;
uint16_t SizeDuty_Cycle = 1;
...
static tBleStatus Generic_STM_App_Update_Char_Ext(uint16_t ConnectionHandle,
		uint16_t ServiceHandle, uint16_t CharHandle, uint16_t CharValueLen,
		uint8_t *pPayload);
...
#define COPY_GENERAL_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x40,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_TIME_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x41,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_PWM_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x60,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_FREQUENCY_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x61,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_DUTY_CYCLE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x62,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
static SVCCTL_EvtAckStatus_t Custom_STM_Event_Handler(void *Event)
{
	...
	case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
		...
		case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
							...
							+ CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
			{
				...
				case (!(COMSVC_Notification)):
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_FREQUENCY_NOTIFY_DISABLED_EVT;
				...
				case COMSVC_Notification:
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_FREQUENCY_NOTIFY_ENABLED_EVT;
					Custom_STM_App_Notification(&Notification);
							...
							+ CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
			{
				...
				case (!(COMSVC_Notification)):
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_DUTY_CYCLE_NOTIFY_DISABLED_EVT;
					Custom_STM_App_Notification(&Notification);
				...
				case COMSVC_Notification:
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_DUTY_CYCLE_NOTIFY_ENABLED_EVT;
					Custom_STM_App_Notification(&Notification);
		...
		case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE:
			...
			} /* if (read_req->Attribute_Handle == (CustomContext.CustomFrequencyHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
			else if (read_req->Attribute_Handle
					== (CustomContext.CustomDuty_CycleHdle
							+ CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
			{
...
void SVCCTL_InitCustomSvc(void)
{
	 ...
	 * service_max_attribute_record = 1 for GENERAL +
	 *                                2 for TIME +
	 *                              = 3
	 *
	 ...
	 * service_max_attribute_record = 1 for PWM +
	 *                                2 for FREQUENCY +
	 *                                2 for DUTY_CYCLE +
	 *                                1 for FREQUENCY configuration descriptor +
	 *                                1 for DUTY_CYCLE configuration descriptor +
	 *                              = 7
	 *
	 * This value doesn't take into account number of descriptors manually added
	 * In case of descriptors added, please update the max_attr_record value accordingly in the next SVCCTL_InitService User Section
	...
	UUID_TYPE_128, &uuid, SizeFrequency,
	...
	ret = aci_gatt_add_char(CustomContext.CustomPwmHdle,
	UUID_TYPE_128, &uuid, SizeDuty_Cycle,
	CHAR_PROP_READ | CHAR_PROP_NOTIFY,
	ATTR_PERMISSION_NONE,
	GATT_NOTIFY_ATTRIBUTE_WRITE | GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
			0x10,
		...
		uint8_t *pPayload)
{
	...
	case CUSTOM_STM_TIME:
		ret = aci_gatt_update_char_value(CustomContext.CustomGeneralHdle,
				CustomContext.CustomTimeHdle, 0, /* charValOffset */
	...
	case CUSTOM_STM_FREQUENCY:
		ret = aci_gatt_update_char_value(CustomContext.CustomPwmHdle,
				CustomContext.CustomFrequencyHdle, 0, /* charValOffset */
				SizeFrequency, /* charValueLen */
				(uint8_t*) pPayload);
		if (ret != BLE_STATUS_SUCCESS)
		{
	...
	case CUSTOM_STM_DUTY_CYCLE:
		ret = aci_gatt_update_char_value(CustomContext.CustomPwmHdle,
				CustomContext.CustomDuty_CycleHdle, 0, /* charValOffset */
				SizeDuty_Cycle, /* charValueLen */
				(uint8_t*) pPayload);
		if (ret != BLE_STATUS_SUCCESS)
		{
...
tBleStatus Custom_STM_App_Update_Char_Variable_Length(
		Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload, uint8_t size)
{
	tBleStatus ret = BLE_STATUS_INVALID_PARAMS;
 ...
 *
 */
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle,
		Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
	tBleStatus ret = BLE_STATUS_INVALID_PARAMS;
	...
	case CUSTOM_STM_TIME:
		...
		ret = Generic_STM_App_Update_Char_Ext(Connection_Handle,
				CustomContext.CustomGeneralHdle, CustomContext.CustomTimeHdle,
				SizeTime, pPayload);
	...
	case CUSTOM_STM_FREQUENCY:
		...
		ret = Generic_STM_App_Update_Char_Ext(Connection_Handle,
				CustomContext.CustomPwmHdle, CustomContext.CustomFrequencyHdle,
				SizeFrequency, pPayload);
	...
	case CUSTOM_STM_DUTY_CYCLE:
		...
		ret = Generic_STM_App_Update_Char_Ext(Connection_Handle,
				CustomContext.CustomPwmHdle, CustomContext.CustomDuty_CycleHdle,
				SizeDuty_Cycle, pPayload);
		...
		uint8_t *pPayload)
{
	...
	ret = aci_gatt_update_char_value_ext(ConnectionHandle, ServiceHandle,
			CharHandle, 0, /* update type:0 do not notify, 1 notify, 2 indicate */
	...
	{
		ret = aci_gatt_update_char_value_ext(ConnectionHandle, ServiceHandle,
				CharHandle, 1, /* update type:0 do not notify, 1 notify, 2 indicate */
				CharValueLen, /* charValueLen */
				243, /* value offset */
