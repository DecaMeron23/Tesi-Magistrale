...
static tBleStatus Generic_STM_App_Update_Char_Ext(uint16_t ConnectionHandle, uint16_t ServiceHandle,
		uint16_t CharHandle, uint16_t CharValueLen, uint8_t *pPayload);
...
#define COPY_MY_P2P_SERVER_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xff,0x40,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_MY_LED_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xff,0x41,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_MY_SWITCH_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xff,0x42,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
	...
	PRIMARY_SERVICE, max_attr_record, &(CustomContext.CustomP2PsHdle));
	...
	UUID_TYPE_128, &uuid, SizeLed_C,
	CHAR_PROP_READ | CHAR_PROP_WRITE_WITHOUT_RESP,
	ATTR_PERMISSION_NONE,
	GATT_NOTIFY_ATTRIBUTE_WRITE, 0x10,
	CHAR_VALUE_LEN_VARIABLE, &(CustomContext.CustomLed_CHdle));
	if (ret != BLE_STATUS_SUCCESS)
	{
	...
	ret = aci_gatt_add_char(CustomContext.CustomP2PsHdle,
	UUID_TYPE_128, &uuid, SizeSwitch_C,
	CHAR_PROP_NOTIFY,
	ATTR_PERMISSION_NONE,
	GATT_NOTIFY_ATTRIBUTE_WRITE, 0x10,
	CHAR_VALUE_LEN_VARIABLE, &(CustomContext.CustomSwitch_CHdle));
	if (ret != BLE_STATUS_SUCCESS)
	{
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
	...
	switch (CharOpcode)
	{
		...
		case CUSTOM_STM_LED_C:
			ret = aci_gatt_update_char_value(CustomContext.CustomP2PsHdle, CustomContext.CustomLed_CHdle, 0, /* charValOffset */
		...
		case CUSTOM_STM_SWITCH_C:
			ret = aci_gatt_update_char_value(CustomContext.CustomP2PsHdle, CustomContext.CustomSwitch_CHdle, 0, /* charValOffset */
 ...
 *
 */
tBleStatus Custom_STM_App_Update_Char_Variable_Length(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload,
		uint8_t size)
{
	tBleStatus ret = BLE_STATUS_INVALID_PARAMS;
 ...
 *
 */
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode,
		uint8_t *pPayload)
{
	tBleStatus ret = BLE_STATUS_INVALID_PARAMS;
	...
	switch (CharOpcode)
	{
		...
		case CUSTOM_STM_LED_C:
			...
			ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2PsHdle,
					CustomContext.CustomLed_CHdle, SizeLed_C, pPayload);
		...
		case CUSTOM_STM_SWITCH_C:
			...
			ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2PsHdle,
					CustomContext.CustomSwitch_CHdle, SizeSwitch_C, pPayload);
		...
		uint16_t CharHandle, uint16_t CharValueLen, uint8_t *pPayload)
{
	...
	ret = aci_gatt_update_char_value_ext(ConnectionHandle, ServiceHandle, CharHandle, 0, /* update type:0 do not notify, 1 notify, 2 indicate */
	...
	{
		ret = aci_gatt_update_char_value_ext(ConnectionHandle, ServiceHandle, CharHandle, 1, /* update type:0 do not notify, 1 notify, 2 indicate */
		CharValueLen, /* charValueLen */
		243, /* value offset */
