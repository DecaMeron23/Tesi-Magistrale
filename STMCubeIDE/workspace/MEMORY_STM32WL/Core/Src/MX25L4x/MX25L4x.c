...
{
	//TODO da completare il write

	*address = memoryContext.addrFreeSpace;
	return MEM_Write(pData, size, *address);
...
bool MEM_Write(uint8_t *pData, uint16_t size, uint32_t addr)
{
	...
	uint8_t *cmdWRITE  = malloc((size_t) (size + 1 + 3));

	// TODO da verificare se non è null
	...
	isOK &= SPI_Transmit(cmdWRITE, size, MX25L4_MAX_RW_TIME);
