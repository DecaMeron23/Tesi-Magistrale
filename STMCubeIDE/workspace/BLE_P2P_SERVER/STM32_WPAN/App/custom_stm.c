...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomP2P_ServiceHdle;                    /**< P2P_SERVICE handle */
  uint16_t  CustomLed_Ctrl_CharHdle;                  /**< LED_CONTROL_CHAR handle */
  uint16_t  CustomNotify_Push_Btn_CHdle;                  /**< NOTIFY_PUSH_BTN_CHAR handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeLed_Ctrl_Char = 2;
uint16_t SizeNotify_Push_Btn_C = 2;
...
#define COPY_LED_CONTROL_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x41,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_NOTIFY_PUSH_BTN_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0x42,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for P2P_SERVICE +
   *                                2 for LED_CONTROL_CHAR +
   *                                2 for NOTIFY_PUSH_BTN_CHAR +
   *                                1 for NOTIFY_PUSH_BTN_CHAR configuration descriptor +
   *                              = 6
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomP2P_ServiceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: P2P_SERVICE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: P2P_SERVICE , handle = 0x%04x \n\r", CustomContext.CustomP2P_ServiceHdle);
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomP2P_ServiceHdle,
                          ...
                          &(CustomContext.CustomLed_Ctrl_CharHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : LED_CTRL_CHAR, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : LED_CTRL_CHAR , handle = 0x%04x \n\r", CustomContext.CustomLed_Ctrl_CharHdle);
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomP2P_ServiceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeNotify_Push_Btn_C,
                          CHAR_PROP_NOTIFY,
                          ATTR_PERMISSION_NONE,
                          GATT_NOTIFY_ATTRIBUTE_WRITE,
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomNotify_Push_Btn_CHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : NOTIFY_PUSH_BTN_C, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : NOTIFY_PUSH_BTN_C , handle = 0x%04x \n\r", CustomContext.CustomNotify_Push_Btn_CHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LED_CTRL_CHAR:
      ret = aci_gatt_update_char_value(CustomContext.CustomP2P_ServiceHdle,
                                       CustomContext.CustomLed_Ctrl_CharHdle,
                                       0, /* charValOffset */
                                       SizeLed_Ctrl_Char, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value LED_CTRL_CHAR command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value LED_CTRL_CHAR command\n\r");
      }
    ...
    case CUSTOM_STM_NOTIFY_PUSH_BTN_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomP2P_ServiceHdle,
                                       CustomContext.CustomNotify_Push_Btn_CHdle,
                                       0, /* charValOffset */
                                       SizeNotify_Push_Btn_C, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value NOTIFY_PUSH_BTN_C command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value NOTIFY_PUSH_BTN_C command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_LED_CTRL_CHAR:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2P_ServiceHdle, CustomContext.CustomLed_Ctrl_CharHdle, SizeLed_Ctrl_Char, pPayload);
    ...
    case CUSTOM_STM_NOTIFY_PUSH_BTN_C:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomP2P_ServiceHdle, CustomContext.CustomNotify_Push_Btn_CHdle, SizeNotify_Push_Btn_C, pPayload);
